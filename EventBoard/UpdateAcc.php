...
$fetch= $_SESSION['email'];
if($fetch!="")
{

    $call = new Dbhelper();
    $getUser = $call->getUserInformation($fetch);
    foreach($getUser as $something)
    {
        $id= $something->id;
        $fname = $something->fname;
        $lname =  $something->lname;
        $email= $something->email;
        $pass = $something->password;
        $pno = $something->phoneno;
    }
...
if(isset($_POST['update']))
{
    ...
    $validateEmail = $user->validateEmail1($email);
    ...
    {
        $update = new Dbhelper();
        $count = $update->update($id,$fname,$lname,$email,$pno,$pass);
        if($count==1)
        {
...
<main class="container-fluid text-center">
    ...
    <form class="signup" method="post" action="">
        ...
        <h1 class="h3 mb-3 font-weight-normal">Update Information</h1>
        <!--        Building Forms in the format of Table Using Bootstrap Classes-->
        <div class="form-row" name="row-1">
            <div class="form-group col-md-6">
                <input  class= "sr-only" type="text" id="inputFirstName" class="form-control" placeholder="First Name"  autofocus="" name="id" value="<?php echo $id;?>" hidden>
                ...
                <?php echo "<h6 style='color:red'>".$firstName."</h6>";
                ?>
            ...
            <div class="form-group col-md-6">
                ...
                <?php echo "<h6 style='color:red'>".$lastName."</h6>";?>
            </div>
        </div>
        <div class="form-row" name="row-2">
            <div class="form-group col-md-12">
                ...
                <?php echo "<h6 style='color:red'>".$validateEmail."</h6>";?>
            </div>
        </div>
        <div class="form-row" name="row-2">
            <div class="form-group col-md-12">
                ...
                <?php echo "<h6 style='color:red'>".$validatePhone."</h6>";?>
            </div>
        </div>
        <div class="form-row" name="row-3">
            <div class="form-group col-md-6">
                <label for="inputPassword" class="sr-only">Password</label>
                <input type="password" id="inputPassword" class="form-control" placeholder="Password"  autofocus="" name="password" value="<?php echo $pass;?>">
                <?php echo "<h6 style='color:red'>".$validatePass."</h6>";?>
            </div>
            <div class="form-group col-md-6">
                <label for="inputCnfPassword" class="sr-only">Confirm Password</label>
                <input type="password" id="inputCnfPassword" class="form-control" placeholder="Confirm Password"  autofocus="" name="cnfPass" value="<?php echo $pass;?>">
